...
( function( document, window ) {
    ...
    var computeWindowScale = function(config) {
        ...
        console.log('Initial scales - hScale:', hScale, 'wScale:', wScale);
    
        var scale = hScale > wScale ? wScale : hScale;
        console.log('Initial scale:', scale);
    
        // 縦にもはみ出た場合にスケーリングする処理を追加
        if (wScale > hScale) {
            scale = hScale;
            console.log('Adjusted scale for vertical overflow:', scale);
        }
    
        if (config.maxScale && scale > config.maxScale) {
            scale = config.maxScale;
            console.log('Scale adjusted to maxScale:', scale);
        }
    
        if (config.minScale && scale < config.minScale) {
            scale = config.minScale;
            console.log('Scale adjusted to minScale:', scale);
        }
